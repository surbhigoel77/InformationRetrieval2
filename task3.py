...
st = time.time()
training_data,validation_data = train_test_split(train_data_sampled,test_size = 0.2,stratify = train_data_sampled['relevancy'])

X_train = training_data[['cosine_similarity','doc_len', 'query_len']]
y_train = training_data['relevancy']

X_validation = validation_data[['cosine_similarity','doc_len', 'query_len']]
y_validation = validation_data['relevancy']


train_qid_group = training_data.groupby(training_data.qid).size().values
valid_qid_group = validation_data.groupby(validation_data.qid).size().values

# Set up the XGBoost DMatrix for training and validation
dtrain = xgb.DMatrix(data = X_train, label=y_train)
dtrain.set_group(train_qid_group)
dvalid = xgb.DMatrix(data = X_validation, label=y_validation)
dvalid.set_group(valid_qid_group)
...
# Hyperparameter Tuning and fit the training data
best_score = 0.0
best_params = {}
...
for max_depth in param_grid['max_depth']:
    for eta in param_grid['eta']:
        params = {'max_depth': max_depth,
                  'eta': eta,
                  'objective': 'rank:ndcg'}
        xgbmodel = xgb.XGBRanker(**params)
        xgbmodel.fit(X_train, y_train, group = train_qid_group, eval_set=[(X_validation, y_validation)],eval_group=[valid_qid_group], eval_metric='ndcg')
        ndcg = xgbmodel.evals_result()['validation_0']['ndcg'][-1]
...
print("Time taken to run xgboost: ", time.time() - st)
# print best hyperparameters and their score
print('Best hyperparameters:', best_params)
print('Best score:', best_score)
best_model.save_model('lamdamart.model')
...
valid_data['relevancy_prediction'] = best_model.predict(valid_data[['cosine_similarity','doc_len', 'query_len']])
...
lambdam_predictions = best_model.predict(X_test)
test_data['LM_score'] = lambdam_predictions
test_data['LM_Rank'] = test_data.groupby('qid')['LM_score'].rank(method='first',ascending=False)
test_data.sort_values(by=['qid','LM_Rank'])
    ...
    with open(filename, 'w') as csvfile:
        ...
        for q in querylist:
            ...
            for i, row in sorted_df.iloc[:limit].iterrows():
                writer.writerow((int(row.qid),'A2',int(row.pid),row[col],round(row.LM_score,8),'LM'))

write_results('LM', test_data)
print("Total time taken in task3: ", time.time() - start)